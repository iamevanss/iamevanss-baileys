...
export async function resolveRealJid(sock, raw, metadata = {}) {
    ...
    // Prefer explicit phone-number alternatives supplied by WhatsApp.
    const alternatives = [
    ...
    if (isLidUser(jid)) {
        ...
        // For self-messages, WhatsApp may provide the LID without a usable
        // remoteJidAlt. The authenticated account's PN is authoritative here.
        if (metadata?.fromMe) {
            const ownId = sock?.user?.id || sock?.authState?.creds?.me?.id;
            if (typeof ownId === 'string') {
                const ownJid = jidNormalizedUser(ownId);
                if (isPnUser(ownJid)) {
                    const phone = ownJid.split('@')[0].replace(/\D/g, '') || ownJid.split('@')[0];
                    if (phone.length >= 7) {
                        return { jid: ownJid, phone, resolved: true, lid: jid };
                    }
                }
            }
        }
    ...
    const phone = jid.split('@')[0].replace(/\D/g, '') || jid.split('@')[0];
    return { jid, phone, resolved: false, lid: isLidUser(jid) ? jid : undefined };
}
